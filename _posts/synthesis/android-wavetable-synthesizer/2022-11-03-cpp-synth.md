...
6. [Wavetable Synthesis Algorithm in C++ (this one)]({% post_url synthesis/android-wavetable-synthesizer/2022-11-03-cpp-synth %})
...
4. Created an `AudioPlayer` class that uses the Oboe library to handle Android’s audio deivce.
...
- [Wavetable Synthesis Theory,](https://thewolfsound.com/sound-synthesis/wavetable-synthesis-algorithm/)
- [Wavetable Synthesizer VST Plugin Tutorial in C++ u](https://www.notion.so/Article-d7475c8ab7bd437d8cc1470960dae4e0)[sing JUCE.](https://thewolfsound.com/sound-synthesis/wavetable-synth-plugin-in-juce/)
...
5. [Triangle, Square, Saw Generation](#triangle-square-saw-generation)
   1. [Changes to CMakeLists.txt](#changes-to-cmakeliststxt)
6. [WavetableOscillator Class](#wavetableoscillator-class)
...
7. [Changes to WavetableSynthesizer](#changes-to-wavetablesynthesizer)
...
8. [Testing the Synthesizer](#testing-the-synthesizer)
9. [Android Wavetable Synthesizer App Summary](#android-wavetable-synthesizer-app-summary)
...
Both of these will be controlled by the `WavetableSynthesizer` class that we alreade have.
...
If you are unsure what these formulas do, please, check out [my tutorial on basic waveforms in sound synthesis](https://thewolfsound.com/sine-saw-square-triangle-pulse-basic-waveforms-in-synthesis/) and the supporting [Python code](https://github.com/JanWilczek/wolf-sound-blog/blob/master/_py/posts/synthesis/2022-06-26-sine-saw-square-triangle-basic-waveforms-in-synthesis/main.py).

The formulas are taken from a great book by professor Marek Pluta:

Marek Pluta, *Sound Synthesis for Music Reproduction and Performance*, monograph, AGH University of Science and Technology Press, Kraków 2019.
...
In the [previous part of the tutorial](https://thewolfsound.com/android-synthesizer-5-android-audio-with-c-plus-plus-and-oboe/), we have implemented an `A4Oscillator` class that playes back a sine at 440 Hz. We did that to test our audio output.
...
In the Listing ???, there is the class declaration of `WavetableOscillator`, which should be put in the *include/WavetableOscillator.h* file.
...
`interpolateLinearly()` member function is used in the wavetable synthesis algorithm. It has already been presented in the tutorial on the [wavetable synthesizer JUCE plugin](https://thewolfsound.com/sound-synthesis/wavetable-synth-plugin-in-juce/).
...
`index` member is the real-valued index into the wavetable.

`indexIncrement` is the amount by which we increase the `index` after every sample generation. It depends on the frequency, sample rate, and the buffer size. It atomic so that it can be set in a thread-safe way.

`sampleRate` and `amplitude` are self-explanatory.

The remaining members are used to implement thread-safe wavetable swapping.

### Included Headers and the Constructor

In Listing ???, you can see the headers included in the *WavetableOscillator.cpp* file and the definition of the constructor.
...
In Listing ???, the `getSample()` method is shown along with `interpolateLinearly()`.
...
The rest of the function is the same algorithm as in the [wavetable synthesizer JUCE plugin tutorial](https://thewolfsound.com/sound-synthesis/wavetable-synth-plugin-in-juce/):

1. We read the index into the wavetable. Modulo operation always returns a value withing the [0, `waveTable.size()`) range (we **loop** over the wavetable).
2. We interpolate linearly the value of the wavetable at that real-valued index using the nearest neighbors (upper and lower integer indices into the wavetable). This is simple proportional weighing of samples from the wavetable.
3. We increment the index by the current index increment.
4. We return the sample scaled by the current amplitude.

### Wavetable Swapping

In Listing ???, the wavetable swapping logic is shown. These functions are quite complicated so allow me a more detailed explanation. However, if you are not interested in this part, you can skip to the next.
...
- `std::atomic<bool> swapWavetable{false};`, which is a boolean flag that indicates that the `waveTable` member should be swapped with `wavetableToSwap`.
- `std::atomic<bool> wavetableIsBeingSwapped{false};`, which is a boolean flag that indicates that a swap is being performed and, thus, `wavetableToSwap` should not be used.
...
3. In the body of the if-statement, it:
    ...
    2. sets the `swapWavetable` flag to false (because no swap is needed).
4. Indicates that a swap has finished and the `wavetableToSwap` can be accessed again.
...
In Listing ???, there are methods `setFrequency()` and `setAmplitude()` presented.
...
`setFrequency()` calculates the current index increment based on the given frequency, wavetable size, and the sample rate. If you do not understand this formula, I explained it in the [wavetable synthesis algorithm tutorial.](https://thewolfsound.com/sound-synthesis/wavetable-synthesis-algorithm/)
...
In `onPlaybackStopped()` (Listing ???), we simply reset the index member to 0 so that it starts playing from the beginning of the wavetable upon the next playback start.
...
And that it for the `WavetableOscillator` class! The last class to change is the `WavetableSynthesizer` and then we’re done!

## Changes to WavetableSynthesizer

To make it clear what happened to the `WavetableSynthesizer` class, I put its whole declaration in Listing ???.
...
Below I have put the implementations of methods that changed since the last tutorial episode. That includes all but the destructor and `isPlaying()`.

### Included Files and the Constructor

In Listing ???, you can see the included files and the constructor of `WavetableSynthesizer`.
...
The important bit is the initialization of the `_oscillator` member. As you can see, we pass a generated wavetable to its constructor. We generate it by calling `_wavetableFactory.getWaveTable(_currentWavetable)` , where `*_*currentWavetable` is the value of the `Wavetable` enum class.

We can use `_wavetableFactory` and `_currentWavetable` here because they are placed before `_oscillator` in the member declaration list of the class (Listing ???).

### play() and stop()

In Listing ???, there are `play()` and `stop()` member functions. They are introduced to protect the access to the `_audioPlayer` because these member functions may be called simultaneously from different threads and `_audioPlayer` is not thread-safe. `_isPlaying` is atomic so it should be safe to use. However, we change it under the mutex to ensure that it’s state is consistent, in other words, the `_mutex` ensures that subsequent calls to `play()` and `stop()` are executed sequentially.
...
In Listing ???, there are frequency and amplitude setters presented.
...
You should now be able to compile the whole project and install it on your device of choice.
...
4. how to call C++ code from Kotlin on Android using Gradle and CMake,
5. how to play back sound on Android using the Oboe library, and
6. how to generate sound using the wavetable synthesis in C++.

Congratulations on completing this mini-series! You are now prepared to tackle more complicated Android applications.

If you would like to learn more about audio programming, be sure to sign up on my [newsletter](https://thewolfsound.com/newsletter/), where I share all the newest content as well as useful tips and tricks regarding learning audio programming.
