...
5. [Playing Back Audio on Android with C++ (this one)]({% post_url synthesis/android-wavetable-synthesizer/2022-10-23-oboe %})
...
2. write a callback for the Android audio driver using one of the available audio APIs.
...
The second one is a more modern approach containing useful features for professional audio apps. However, it is only supported from Android API level ???, which means it will not run on older Android devices.
...
[Google recommends to use Oboe.](https://developer.android.com/games/sdk/oboe)
...
1. start output playback stream, `play()`,
2. stop output playback stream, `stop()`, and
3. user-defined **audio callback**.
...
Since the callback must return a specified number of samples in time much shorter than these samples represent.

That means that never ever can block the audio thread. In particular we **mustn’t** do the following on the audio thread:
...
If we take too much times to generate the samples, and **underrun** will occurr, which manifests itself through glitches in the audio output.
...
To connect to Android’s audio driver using the Oboe library, we need to do implement the audio stream setup, starting, stopping, and the audio callback. We can do it by subclassing `oboe::AudioStreamDataCallback`.
...
_Listing {% increment listingId20221023 %}._
```cpp
#pragma once
...
This interface can now be implemented by a class that underneath uses on of the 3 previously described audio APIs. Since we only interact with the audio device through the interface, we can completely abstract out which particular audio API is used.

Any implementer of `AudioPlayer` has to provide a function for the audio callback but this does not have to be part of the interface. However, any audio callback will need to access our sound-generation code. It will do so through another interface, `AudioSource`.
...
In Listing ???, you can see its class declaration.
...
We need to include the *Oboe.h* header here because we cannot inherit from a class we don’t have full declaration of (an incomplete class).
...
int32_t OboeAudioPlayer::play() {
  ...
  if (result != Result::OK) {
    // indicate that stream creation failed
...
DataCallbackResult OboeAudioPlayer::onAudioReady(oboe::AudioStream* audioStream,
  ...
  for (auto frame = 0; frame < framesCount; ++frame) {
    // retrieve a sample from the AudioSource (in our case, it's a WavetableOscillator)
...
Finally, we need to connect the player, the oscillator, and the `WavetableSynthesizer` class that we created in the [previous part of the tutorial.]({% post_url synthesis/android-wavetable-synthesizer/2022-10-09-jni %})
...
We also define the desired sampling rate we want to use as a compile-time constant. 48000 Hz is a typical value in this scenario.

Finally, we declare 2 members: a `shared_ptr` to an `AudioSource` and a `unique_ptr` to an `AudioPlayer`. `AudioSource` must be used with a shared pointer because we pass it to the `AudioPlayer`.
...
1. We initialize the member variables in the constructor. Note that the oscillator is passed to the player as an `AudioSource`.
...
1. We modify the `play()` member function of the `WavetableSynthesizer`. If 0 is returned (=success), we update the play state. If not, we log an error.
...
1. We modify the `stop()` member function of the `WavetableSynthesizer`; we stop the player and update the play state.
...
The `${log-lib}` part was explained in the [previous part of the tutorial.]({% post_url synthesis/android-wavetable-synthesizer/2022-10-09-jni %})
