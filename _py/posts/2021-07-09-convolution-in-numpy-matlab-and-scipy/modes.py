...
def main():
    ...
    N = 10
    n_x = np.arange(0, 2*N)
    x = np.sin(2 * np.pi * n_x / 20)
    n_y = np.arange(0, N)
    y = np.exp(0.05 * n_y) - 1.4

    stem(n_x, x, 'x', xlabel='n', ylabel='x[n]')
    stem(n_y, y, 'y', xlabel='n', ylabel='y[n]')

    full = np.convolve(x, y, 'full')
    n_full = np.arange(0, full.shape[0])
    valid = np.convolve(x, y, 'valid')
    n_valid = np.arange(0, valid.shape[0])
    same = np.convolve(x, y, 'same')
    n_same = np.arange(0, same.shape[0])

    stem(n_full, full, 'xy_full', xlabel='n', ylabel=r'$(x \ast y)_{full}$[n]')
    stem(n_valid, valid, 'xy_valid', xlabel='n', ylabel=r'$(x \ast y)_{valid}$[n]')
    stem(n_same, same, 'xy_same', xlabel='n', ylabel=r'$(x \ast y)_{same}$[n]')
