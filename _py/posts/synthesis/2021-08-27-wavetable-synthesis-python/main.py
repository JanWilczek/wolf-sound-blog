...
def fade_in_out(signal, fade_length=1000):
    """Apply a half-cosine window to first and last fade_length samples of signal."""
...
class WavetableOscillator:
    ...
    To this end, it needs a wavetable, the sampling rate and the interpolation method used.
    """
...
def generate_wavetable(length, f):
    """
    Generate a wavetable of specified length using function f(x) where x is phase.
...
def generate_gaussians_table(wavetable_size):
    ...
    # Smooth out at the edges so that the wave table starts and ends around 0.0
...
class Voice:
    ...
    def synthesize(self, frequency, duration_seconds):
        """Generate duration_seconds of samples at the specified frequency."""
